...
class SaveAnalyzedBookmarkUseCase(
    ...
    suspend operator fun invoke(
        ...
        val analysisResult =
            bookmarkRepository.analyzeUrl(url).getOrElse { error ->
        ...
        if (analysisResult.isNewFolder) {
            val newFolder =
                bookmarkRepository.createFolder(analysisResult.suggestedFolder).getOrElse { error ->
                    return Result.failure(Exception("Ошибка создания предложенной папки: ${error.message}"))
                }
            finalFolderId = newFolder.id
        } else {
            val folders =
                bookmarkRepository.getFolders().getOrElse { error ->
