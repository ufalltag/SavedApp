...
class HomeViewModel(
                ...
                .onSuccess { result ->
                    when {
                        ...
                        result.isNewFolder -> {
                            ...
                            saveAnalyzedBookmarkUseCase(url)
                                .onSuccess {
                                    reduce { state.copy(isAnalyzing = false) }
                                    loadHomeData()
                                }.onFailure { error ->
    ...
    fun saveToNewFolder(
        ...
        createFolderUseCase(folderName)
            .onSuccess { folder ->
                saveAnalyzedBookmarkUseCase(url, targetFolderId = folder.id, bookmarkTitle = bookmarkTitle)
                    .onSuccess {
                        reduce { state.copy(isAnalyzing = false) }
                        loadHomeData()
                    }.onFailure { error ->
    ...
    fun saveToExistingFolder(
        ...
        reduce { state.copy(isAnalyzing = true) }
        saveAnalyzedBookmarkUseCase(url, targetFolderId = folderId)
            .onSuccess {
                reduce { state.copy(isAnalyzing = false) }
                loadHomeData()
            }.onFailure { error ->
