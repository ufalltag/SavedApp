...
class BookmarksViewModel(
        ...
        intent {
            ...
            analyzeUrlUseCase(url)
                .onSuccess { result ->
                    val folderId =
                        state.folders
                            .firstOrNull { it.name.equals(result.suggestedFolder, ignoreCase = true) }
                            ?.id
                    ...
                    saveAnalyzedBookmarkUseCase(url = url, folderId = folderId, title = result.title)
                        .onSuccess {
                            reduce { state.copy(isAnalyzing = false) }
                    ...
                    reduce { state.copy(isAnalyzing = false) }
                    postSideEffect(BookmarksSideEffect.ShowToast(error.message ?: "Ошибка анализа ссылки"))
                }
        }
    ...
    fun moveBookmark(
        ...
        updateBookmarkUseCase(bookmarkId, folderId = targetFolderId)
            .onSuccess {
                reduce { state.copy(bookmarks = state.bookmarks.filter { it.id != bookmarkId }) }
    ...
    fun renameBookmark(
        ...
        updateBookmarkUseCase(bookmarkId, title = newTitle)
            .onSuccess {
