...
class BookmarkRepositoryImpl(
    ...
    override suspend fun getFolders(
        page: Int,
        limit: Int,
    ): Result<List<Folder>> = api.getFolders(page, limit).map { dto -> dto.folders.orEmpty().map { it.toDomain() } }
    ...
    override suspend fun renameFolder(
        folderId: String,
        name: String,
    ): Result<Unit> = api.renameFolder(folderId, name)

    override suspend fun deleteFolder(folderId: String): Result<Unit> = api.deleteFolder(folderId)
    ...
    override suspend fun saveBookmark(
        ...
        folderId: String,
        title: String,
    ): Result<Bookmark> = api.createBookmark(url, folderId.toInt(), title).map { it.bookmark.toDomain() }

    override suspend fun updateBookmark(
        bookmarkId: String,
        title: String?,
        folderId: String?,
    ): Result<Unit> = api.updateBookmark(bookmarkId, title, folderId?.toInt())
